...
class AuthService {
    ...
    async login(data: any) {
        try {
            const tokens = (await this.api.post(import.meta.env.VITE_BACKEND_URL0+"/user/login", data));
            return tokens.data;
    ...
    async register(data: any) {
        try {
            const resp = (await this.api.post(import.meta.env.VITE_BACKEND_URL0+"/user/register", data));
            return resp.data;
        } catch (err: any) {
    ...
    async getMe(token: string) {
        return await axios.get(import.meta.env.VITE_BACKEND_URL0+"/auth/me", {
          headers: {
